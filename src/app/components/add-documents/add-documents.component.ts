...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, CardModule, FormsModule, ReactiveFormsModule, DropdownModule, FileUploadModule, ButtonModule, CalendarModule, CheckboxModule, AutoCompleteModule, MessagesModule, ConfirmDialogModule, ProgressSpinnerModule, InputTextModule],
...
export class AddDocumentsComponent {
  ...
  constructor(private documentsService: DocumentService, private route: ActivatedRoute, private confirmationService: ConfirmationService, private messageService: MessageService, private router: Router, private customsDataService: CustomsDataService) { }
            ...
            this.documentsService.uploadDocument(formData).subscribe(
              (response: any) => {
                ...
                if (response.url) {
                  ...
                  //save doc and doc attributes to DB
  ...
  async saveAndSendDocument() {
    this.isSaveAndSend = true
    try {
      ...
      formData.append('DocumentType', this.selectedDocumentCode.code);
      // if (this.documentAttributes && this.documentAttributes.length) {
      //   this.documentAttributes.forEach(attribute => {
      //     if (attribute.value === undefined) {
      //       if (attribute.type === 'Boolean') {
      //         formData.append(`attributes[${attribute.id}]`, JSON.stringify({
      //           id: attribute.id,
      //           value: 'false'
      //         }));
      //       }
      ...
      //       formData.append(`attributes[${attribute.id}]`, JSON.stringify({
      //         id: attribute.id,
      //         value: attribute.value.code ? attribute.value.code : attribute.value
      //       }));
      //     }
      //   });
      // }
  ...
  // validateMetadata() {
  //   for (const attr of this.documentAttributes) {
  //     if (attr.type !== 'Boolean' && attr.isMandatory == true && !attr.value) {
  //       this.msgs1 = [
  //         { severity: 'error', summary: 'Error', detail: ' יש למלא את ערכי המטה דאטה' }
  //       ]; return false;
  //     }
  //   }
  //   return true;
