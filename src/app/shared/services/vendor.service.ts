...
export class VendorService {
  ...
  getVendors$(vendor: any): Observable<any> {
    const body = new HttpParams()
      .set('vendorName', vendor.VendorName)
    ...
    return this.http.post(`${apiConfig.customsApiUrl}Vendor`, body.toString(), {
  ...
  postVendor$(vendor: any): Observable<any> {
    console.log(vendor);
    return this.http
      .post<any>(`${apiConfig.customsdbApiUrl}vendors/PostVendors`, vendor)
      .pipe(
        catchError((error) => {
  ...
  updateVendor$(vendor: any): Observable<any> {
    return this.http.put<any>(
      `${apiConfig.customsdbApiUrl}Vendors/${vendor.VendorID}`,
      vendor,
    );
  }
}
